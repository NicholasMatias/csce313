...
// Timer handler for step completion.
static void timerHandler(int sig, siginfo_t* si, void* uc)
{
    (void)sig;
    (void)uc;
    ...
    // Section 2 - Complete the step
    comp_item->PrintComplete();
    completedSteps->push_back(comp_item->id);
    completeCount++;

    // Trigger dependency removal
    kill(getpid(), SIGUSR1);
}

// Dependency removal handler
void RemoveDepHandler(int sig) {
    (void)sig;
...
// Main function: associates signals, initializes the recipe steps, and manages timers
int main(int argc, char** argv)
{
    string input_file = ProcessArgs(argc, argv);
    if (input_file.empty()) {
    ...
    struct sigaction sa_dep;
    sa_dep.sa_handler = RemoveDepHandler;
    sigemptyset(&sa_dep.sa_mask);
    sigaction(SIGUSR1, &sa_dep, NULL);

    // Continuously check if steps are ready to start and create timers for them.
